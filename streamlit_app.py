...
def init_db():
    with get_conn() as conn:
        ...
        # طلبات الحجز / Leads (للسفر، الطيران، القطار، العمرة، المستثمرين...)
        cur.execute(
            """
        ...
        # تعبئة أولية للأنشطة لو الجدول فاضي (كتالوج ترفيهي أولي)
...
def page_booking_requests():
    ...
    with tab_new:
        ...
        with st.form("new_booking_request"):
            ...
            source = st.selectbox(
                "مصدر الطلب",
                ["Web", "Mobile", "Agent", "Flights", "Rail", "Umrah/Hajj", "Investor", "Other"],
            )
            status = st.selectbox(
                "حالة الطلب",
...
# ==============================
# 6) صفحات الطيران، القطار، العمرة، المستثمرين
# ==============================
